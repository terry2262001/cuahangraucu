...
class UserModel
{
	...
	{
		$dbConnect = new MySQLUtils();
		$query = "SELECT userid, username, password, fullname, phone, status, email,isAdmin from user where userid=:userid";
	...
	// {
	// 	$dbConnect = new MySQLUtils();
	// 	$query = "UPDATE user set username=:username, password=:password, fullname=:fullname, phone=:phone, status:=status, email:=email ,isAdmin:=isAdmin where userid=:userid";
	// 	$param = array(":username" => $this->getUsername(), ":password" => $this->getPassword(), ":fullname" => $this->getTenkhachhang(), ":phone" => $this->getPhone(), ":status" => $this->getStatus(), ":email" => $this->getEmail(),":userid" => $this->getUserid(),":isAdmin"=>$this->getIsAdmin());
	...
	public function updateUser()
	{
		$dbConnect = new MySQLUtils();
		$query = "UPDATE user set username=:username, password=:password,fullname=:fullname,phone=:phone,status=:status, email=:email, isAdmin=:isAdmin where userid=:userid";
		$param = array(":username" => $this->getUsername(), ":password" => $this->getPassword(),":fullname" => $this->getTenkhachhang(),":phone" => $this->getPhone(),":status" => $this->getStatus(), ":email" => $this->getEmail(),":isAdmin"=>$this->getIsAdmin(),":userid" => $this->getUserid());
		$dbConnect->updateData($query, $param);
		$dbConnect->disconnectDB();
	...
	// 	$dbConnect = new MySQLUtils();
	// 	$query = "UPDATE user set username=:username, password=:password,fullname=:fullname,phone=:phone,status=:status, email=:email ,isAdmin=:isAdmin where userid=:userid";
	// 	$param = array(":isAdmin"=>$this->getIsAdmin(),":userid" => $this->getUserid());
	// 	$dbConnect->updateData($query, $param);
	// 	$dbConnect->disconnectDB();
